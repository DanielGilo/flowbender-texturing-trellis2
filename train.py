...
# Reduces CUDA memory fragmentation to help prevent OOM
...
# Force early load of C++ extensions to prevent ABI symbol loss
import cumesh
...
def main(local_rank, cfg):
    ...
    # Build trainer. Constructed before the model summary below since LoRA injection
    # (which changes the model's parameter count) happens in the trainer's constructor.
    ...
    if rank == 0:
        if hasattr(trainer, 'val_dataset') and trainer.val_dataset is not None:
            ...
            print(f'Training examples: {len(trainer.dataset)}')
            print(f'Validation examples: {len(trainer.val_dataset)}')
        else:
