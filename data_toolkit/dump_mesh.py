...
def _install_blender():
    ...
    # No sudo access, so the apt-get install above silently failed? Blender's runtime
    # libs (libxkbcommon, libXrender, ...) also resolve via the active conda env if
    # installed there instead, with no sudo needed:
...
def _dump_mesh(file_path, metadatum, root):
    # Compatibility Patch: Handle TRELLIS 1 format (String) vs TRELLIS 2 format (Dict)
