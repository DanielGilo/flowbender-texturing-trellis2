...
def init_render(engine='CYCLES', resolution=512):
    ...
    if engine == 'CYCLES':
        ...
        # Configure EEVEE for speed and reasonable quality
        try:
...
def init_hdri_lighting(hdri_path: str) -> None:
    """Loads a specific HDRI to use as the absolute ground truth light source."""
    # 1. Clear existing physical lights
    for obj in [o for o in bpy.context.scene.objects if o.type == 'LIGHT']:
        bpy.data.objects.remove(obj, do_unlink=True)
    
    # 2. Setup environment world
    if bpy.context.scene.world is None:
        world = bpy.data.worlds.new("World")
        bpy.context.scene.world = world
    else:
    ...
    # Load the image (ensure this path is correct relative to where you run the script)
    try:
        env_tex.image = bpy.data.images.load(hdri_path)
    except RuntimeError:
        print(f"CRITICAL ERROR: Could not find HDRI at {hdri_path}")
        sys.exit(1)
    ...
    # 4. Color Space Alignment (Crucial for matching PyTorch)
    # This disables Blender's AgX tonemapper, preventing the "washed out" pastel look.
...
def main(arg):
    ...
    for i, view in enumerate(views):
        ...
        metadata = {
            ...
            # We just record that it used the static forest env
