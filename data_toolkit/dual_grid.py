...
if __name__ == '__main__':
    ...
    # process objects. Bypasses dataset_utils.foreach_instance to avoid its file-existence checks.
    func = partial(_dual_grid_mesh, root=opt.dual_grid_root, mesh_dump_root=opt.mesh_dump_root)
    print(f"Starting execution with {opt.max_workers or 4} workers...")
    records = []
    
    # We use a standard executor to run the function in parallel
    with ThreadPoolExecutor(max_workers=opt.max_workers or 4) as executor:
        # Submit tasks. We pass 'None' as the first arg (file path) because we load from pickles.
        futures = [executor.submit(func, None, row) for index, row in metadata.iterrows()]
        
        # Monitor progress
        for future in tqdm(as_completed(futures), total=len(futures), desc='Dual griding'):
            records.append(future.result())

    # Convert list of results back to a DataFrame so the rest of the script works
    dual_grids = pd.DataFrame(records)
