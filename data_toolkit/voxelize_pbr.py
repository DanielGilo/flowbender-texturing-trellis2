...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=opt.max_workers or 4) as executor:
        # We pass 'None' as the first arg because we load from pickle files
        futures = [executor.submit(func, None, row) for index, row in metadata.iterrows()]
        
        for future in tqdm(as_completed(futures), total=len(futures), desc='Voxelizing'):
            records.append(future.result())

    # Convert results to DataFrame
    pbr_voxelized = pd.DataFrame(records)
