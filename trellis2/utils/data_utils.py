...
class BalancedResumableSampler(ResumableSampler):
    ...
    def __init__(
        ...
        self.is_subset = isinstance(dataset, torch.utils.data.Subset)
        original_dataset = dataset.dataset if self.is_subset else dataset
        assert hasattr(original_dataset, 'loads'), 'Dataset must have "loads" attribute to use BalancedResumableSampler'
        super().__init__(dataset, shuffle, seed, drop_last)
        self.batch_size = batch_size
        self.loads = original_dataset.loads
    ...
    def __iter__(self) -> Iterator:
        ...
        for i in range(num_batches):
            ...
            if self.is_subset:
                original_batch_indices = [self.dataset.indices[j] for j in batch_indices]
                batch_loads = [self.loads[j] for j in original_batch_indices]
            else:
