...
class RenderAwareFlowEulerSampler(FlowEulerCfgSampler):
    """
    Generate samples from a flow-matching model using Euler sampling with classifier-free guidance,
    updating the condition at each step using a callback.
    """

    def _inference_model(self, model, x_t, t, cond,
                         neg_cond=None, guidance_strength=1.0,
                         grad_cfg_strength=1.0, cached_pred_v_unguided=None,
                         **kwargs):
        if grad_cfg_strength == 1.0 or cached_pred_v_unguided is None:
            return super()._inference_model(model, x_t, t, cond,
                                            neg_cond=neg_cond,
                                            guidance_strength=guidance_strength,
                                            **kwargs)
        # Gradient CFG: one guided forward pass blended with the cached unguided prediction.
        # cached_pred_v_unguided was produced by the denoiser call inside the grad-estimation
        # step (same x_t, zero gradient in concat_cond), so no extra denoiser activation needed.
        pred_guided = super()._inference_model(model, x_t, t, cond,
                                               neg_cond=neg_cond,
                                               guidance_strength=guidance_strength,
                                               **kwargs)
        return grad_cfg_strength * pred_guided + (1 - grad_cfg_strength) * cached_pred_v_unguided
    ...
    def sample(
        ...
        guidance_strength: float = 3.0,
        verbose: bool = True,
        cond_update_fn: Optional[Callable] = None,
        grad_cfg_strength: float = 1.0,
        **kwargs
    ):
        sample = noise
        t_seq = np.linspace(1, 0, steps + 1)
        ...
        for t, t_prev in tqdm(t_pairs, desc="Sampling", disable=not verbose):
            if cond_update_fn is not None:
                result = cond_update_fn(sample, t, cond, neg_cond)
                cond, neg_cond, concat_cond = result[:3]
                kwargs['concat_cond'] = concat_cond
                kwargs['cached_pred_v_unguided'] = result[3] if len(result) > 3 else None
