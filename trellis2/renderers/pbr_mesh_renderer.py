...
def gamma_correction(x: torch.Tensor, gamma: float = 2.2) -> torch.Tensor:
    ...
    Adds a small epsilon before the power to avoid an infinite gradient at x=0,
    which would otherwise destabilize training on very dark inputs.
    """
    return torch.clamp((x + 1e-6) ** (1.0 / gamma), 0.0, 1.0)
...
class PbrMeshRenderer:
    ...
    def render(
        ...
        # We must create a zero value that is mathematically tied to the input mesh
        # so PyTorch can successfully trace the graph backwards without crashing.
        # mesh.vertices usually carries the graph history from the VAE decoder.
        graph_zero = (mesh.vertices.sum() * 0.0) 
        
        # If the mesh has attributes (like textures), they might be the only
        # things carrying gradients depending on the VAE's marching cubes setup.
        if hasattr(mesh, 'attrs') and mesh.attrs is not None:
            graph_zero = graph_zero + (mesh.attrs.sum() * 0.0)
            
        # Ensure it is broadcastable
        graph_zero = graph_zero.view(1, 1, 1).expand(3, resolution, resolution)
        # ---------------------------------------------------------

        if mesh.vertices.shape[0] == 0 or mesh.faces.shape[0] == 0:
            out_dict = edict(
                normal=torch.zeros((3, resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero,
                mask=torch.zeros((resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero[0],
                base_color=torch.zeros((3, resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero,
                metallic=torch.zeros((resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero[0],
                roughness=torch.zeros((resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero[0],
                alpha=torch.zeros((resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero[0],
                clay=torch.zeros((resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero[0],
            )
            for i, k in enumerate(envmap.keys()):
                shaded_key = f"shaded_{k}" if k != '' else "shaded"
                out_dict[shaded_key] = torch.zeros((3, resolution, resolution), dtype=torch.float32, device=self.device) + graph_zero
            return out_dict
        ...
        # Calculate actual Normalized Device Coordinates (NDC) to check for hardware overflow
        xy_vals = vertices_clip[0, :, 0:2]
        w_vals = vertices_clip[0, :, 3:4]
        safe_w = torch.where(w_vals.abs() < 1e-8, torch.tensor(1e-8, device=w_vals.device), w_vals)
        ndc_max = (xy_vals / safe_w).abs().max().item()
        
        # Define the absolute physical limits of the nvdiffrast rasterizer
        is_triangle_soup = num_faces > 10000000 # 10000000
        is_ndc_overflow = ndc_max > 300000
        
        if is_triangle_soup or is_ndc_overflow:
            if is_triangle_soup:
                print(f"Warning: massive triangle soup detected ({num_faces} faces).")
            if is_ndc_overflow:
                print(f"Warning: NDC coordinate overflow (max NDC = {ndc_max:.2f}).")
            print("Bypassing nvdiffrast and returning safe empty gradients.")
        ...
        # Manually compute the norm (rather than F.normalize/torch.norm) to avoid NaNs at exactly 0
        face_normal_sq = torch.sum(face_normal ** 2, dim=1, keepdim=True)
        # Add 1e-8 inside the square root to prevent division by zero AND infinite derivatives
        safe_norm = torch.sqrt(face_normal_sq + 1e-8)
        face_normal = face_normal / safe_norm
        ...
        # Ambient occulusion
        # Wrap with RNG state save/restore to ensure deterministic rendering
