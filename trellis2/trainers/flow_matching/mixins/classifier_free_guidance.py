...
class ClassifierFreeGuidanceMixin:
    # def __init__(self, *args, p_uncond: float, p_cond_no_render: float, **kwargs):
    #     super().__init__(*args, **kwargs)
    #     self.p_uncond = p_uncond
    #     self.p_cond_no_render = p_cond_no_render
        
    def __init__(self, *args, p_uncond: float, p_unguided: float = 0.0, p_cond_no_render: float = 0.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.p_uncond = p_uncond
        self.p_unguided = p_unguided
        self.p_cond_no_render = p_cond_no_render
    

    def get_cond(self, cond, neg_cond=None, p_uncond=None, **kwargs):
        """
        ...
        if p_uncond > 0:
            ...
            mask = list(np.random.rand(B) < p_uncond)
